...
SONG_LYRICS_SYSTEM_TEMPLATE = (
    "Ты ведёшь совместную сессию сочинения ТЕКСТА ПЕСНИ для {names} — веди себя как соавтор-музыкант, "
    "а не просто помощник с промптом.\n"
    "Каждое сообщение в истории подписано именем того, кто его написал.\n"
    "- Сначала, если ещё не ясно, уточни жанр, настроение и тему песни.\n"
    "- Участники пишут строки/куплеты по очереди или предлагают куски — следи за рифмой и ритмом "
    "(количеством слогов), аккуратно подсказывай варианты, если строка не рифмуется или сбивается.\n"
    "- Явно помечай структуру, когда она вырисовывается: [Куплет 1], [Припев], [Куплет 2], [Бридж] и т.д.\n"
    "- Хвали удачные строки коротко и искренне, обращайся к участникам по именам.\n"
    "- Когда текста набралось на полноценную песню, мягко предложи нажать «Готово».\n"
    "Пиши коротко и живо, на русском."
)
...
def get_room_reply(category: str, participant_names: list[str], history: list[dict]) -> str:
    ...
    if category == "lyrics":
        system_prompt = SONG_LYRICS_SYSTEM_TEMPLATE.format(names=names)
    else:
...
def get_room_final_prompt(category: str, history: list[dict]) -> str:
    """Собирает финальный результат из всей истории обсуждения в комнате."""
    if category == "lyrics":
        system_prompt = (
            "На основе всей сессии сочинения ниже собери готовый ТЕКСТ ПЕСНИ целиком: аккуратно оформи "
            "с метками [Verse 1], [Chorus], [Verse 2], [Bridge] и т.д., используя реально написанные "
            "участниками строки (не выдумывай заново, только слегка причёсывай при необходимости, если "
            "чего-то явно не хватает — аккуратно дополни в том же духе). Начни строго со строки "
            "'🎵 ГОТОВЫЙ ТЕКСТ ПЕСНИ:' на отдельной строке. После текста, отдельным абзацем, коротко "
            "предложи жанр/стиль/настроение этой песни для генерации музыки — начни эту строку с "
            "'Стиль для Suno:'."
        )
    else:
        label = ROOM_CATEGORY_LABELS.get(category, ROOM_CATEGORY_LABELS["other"])
        system_prompt = (
            ...
            "нейросети). После промпта можно коротко, в одну строку, пояснить по-русски, что учли из "
            "идей каждого участника."
        )

    messages = [{"role": "system", "content": system_prompt}] + history[-40:]
    try:
        return chat_completion_with_fallback(messages, temperature=0.75, max_tokens=1200)
    except Exception as e:
        logging.exception("Ошибка при составлении финального результата комнаты")
        return describe_groq_error(e)
