...
SONG_EXTRA_GUIDANCE = (
    "\nЭто именно СОЧИНЕНИЕ ПЕСНИ, а не просто промпт — веди себя как соавтор-музыкант:\n"
    "- Уточняй структуру (куплет/припев/бридж), настроение, жанр и темп, если это ещё не ясно.\n"
    "- Когда участники предлагают строки — обращай внимание на рифму и ритм, аккуратно подсказывай "
    "варианты, если что-то не рифмуется или сбивается по слогам.\n"
    "- Поощряй — если строчка получилась классная, скажи об этом прямо, коротко."
)
...
def get_room_reply(category: str, participant_names: list[str], history: list[dict]) -> str:
    ...
    system_prompt = (
        ...
        + (SONG_EXTRA_GUIDANCE if category == "suno" else "")
    )
...
def get_room_final_prompt(category: str, history: list[dict]) -> str:
    ...
    label = ROOM_CATEGORY_LABELS.get(category, ROOM_CATEGORY_LABELS["other"])
    song_note = (
        " Если по ходу обсуждения появлялись конкретные строки/куплеты — включи их в финальный "
        "текст песни как есть, аккуратно дополнив недостающими частями (куплет/припев) в том же духе."
        if category == "suno"
        else ""
    )
    system_prompt = (
